...
class PreprocessDialog(QDialog):
    ...
    def __init__(self, parent, config_base, config_normal):
        ...
        for slot in ("base", "normal"):
            ...
            for w in (alpha_cb, src_combo, src_label, levels_cb, ob_spin, ob_label, ow_spin, ow_label):
                w.setVisible(slot == "base")

        layout.addWidget(self._widgets["base"]["alpha_cb"])
        src_sub = QHBoxLayout()
        src_sub.addWidget(self._widgets["base"]["src_label"])
        src_sub.addWidget(self._widgets["base"]["src_combo"], 1)
        layout.addLayout(src_sub)

        layout.addWidget(self._widgets["base"]["levels_cb"])
        levels_grid = QGridLayout()
        levels_grid.addWidget(self._widgets["base"]["ob_label"], 0, 0)
        levels_grid.addWidget(self._widgets["base"]["ob_spin"], 0, 1)
        levels_grid.addWidget(self._widgets["base"]["ow_label"], 1, 0)
        levels_grid.addWidget(self._widgets["base"]["ow_spin"], 1, 1)
    ...
    def _sync_slot_ui(self):
        ...
        self._hint2.setVisible(sl == "base")
